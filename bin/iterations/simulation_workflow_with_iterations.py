...
def create_workflow(args, submit_sim):
    ...
    for z in z_list:
        for job in range(jobs):
            ...
            simulation_output_filename = f'g4out_{z}_{job:04d}.{args.iteration}.root'
            data_processing_output_filename = f'analysis_{z}_{job:04d}.{args.iteration}.root'
            out_file_histogram_name = f'histograms_{z}_{job:04d}.{args.iteration}.root'
            out_file_histogram_name = f'histograms_{z}_{job:04d}.{args.iteration}.root'
            peak_area_filename = f'peak_ares'
            ...
            data_processing_job = Job(data_processing)\
                .add_args(simulation_output_filename, data_processing_output_filename, out_file_histogram_name)\
                .add_inputs(File(simulation_output_filename), File("AngularCorrelationHelper.cxx"), File("AngularCorrelationHelper.hh")) \
                .add_outputs(File(out_file_histogram_name), stage_out=False)\
                .add_profiles(
                    Namespace.CONDOR,
                    key="+SingularityImage",
            ...
            if merge_job is None:
                merge_job = Job(merge)\
                        .add_args(f'{merge_id}.{args.iteration}.tar.gz')\
                        .add_outputs(File(f'{merge_id}.{args.iteration}.tar.gz'))\
                        .add_profiles(
                            Namespace.CONDOR,
                            key="+SingularityImage",
...
def parse_inputs():
    ...
    parser.add_argument('--iteration', dest='iteration', required=False, type=int, help="Current iteration of same simulation")
...
def main():
    ...
    args = parse_inputs()
    create_workflow(args, submit_sim=args.submit)
